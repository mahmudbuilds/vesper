...
import { Cpu, Sparkles, Check, ChevronDown, Wand2, MessageSquare, RefreshCw } from 'lucide-react';
...
export const Features: React.FC = () => {
  ...
  return (
    <section id="features" className="py-24 max-w-6xl mx-auto px-4 sm:px-6 bg-white overflow-hidden">
      <h2 className="text-3xl sm:text-5xl font-semibold tracking-tight text-[#19191D] mb-12 text-left">
        How Vesper helps during a live interview
      </h2>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Card 1: Vesper listens to the prompt & parses code (Blue Card) */}
        <div className="blue-listen-card p-6 sm:p-8 md:p-9 text-white flex flex-col justify-between min-h-[460px] relative overflow-hidden">
          <div>
          ...
          <div className="my-auto py-8 text-center">
            ...
            <div id="audio-bars-row" className="flex items-center justify-center gap-1.5 mt-8 px-4 h-12">
              {waveHeights.map((h, i) => (
                <div
                  key={i}
                  className="w-1 sm:w-1.5 rounded-full bg-white/70 transition-all duration-75"
                  style={{
                    height: `${h}%`,
          ...
          <div className="h-14 w-full rounded-2xl bg-black/20 backdrop-blur-sm border border-white/10 flex items-center justify-between px-4 text-xs font-sans text-white/80">
            ...
            <span className="text-emerald-300 font-semibold font-mono">Active · 0.04s latency</span>
          </div>
        </div>

        {/* Card 2: When you need help, Vesper assists instantly (Gray Bento Card) */}
        <div className="gray-assist-card p-6 sm:p-8 md:p-9 text-[#19191D] flex flex-col justify-between min-h-[460px] relative overflow-hidden">
          <div>
          ...
          <div className="flex justify-center my-4">
            <div className="flex items-center gap-2 px-3 py-1 rounded-full capsule-pill text-white text-xs shadow-lg border border-white/20">
              ...
              <button className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded bg-white/10">
                <ChevronDown className="size-3.5 text-gray-400" />
                <span>Hide</span>
              </button>
              <button className="size-5 rounded-full bg-white/10 flex items-center justify-center">
          ...
          <div className="cluely-dark-hud p-4 text-white shadow-2xl rounded-2xl">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="px-2.5 py-0.5 rounded-md bg-blue-600 text-white text-[10px] font-semibold">
                O(1) Time
              </span>
            </div>
            <p className="text-xs text-gray-200 leading-relaxed font-mono bg-black/40 p-2 rounded-lg border border-white/10">
            ...
            <div className="rounded-xl border border-white/15 bg-black/40 p-1.5 flex items-center justify-between text-[11px] text-white/60">
              ...
              <span className="size-5 rounded-full bg-blue-600 flex items-center justify-center text-white text-[10px]">
                ▶
              </span>
            </div>
          </div>
